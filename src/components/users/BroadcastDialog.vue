...
<template>
    <Dialog v-model:open="open">
      ...
      <DialogContent class="sm:max-w-[425px]">
        ...
        <div class="border px-3 p-2 rounded-xl">
            <p>Broadcasting to {{ recipients?.length }} Users</p>
        </div>
        ...
        <div class="grid gap-4 py-4">
          ...
          <div class="">
            <div class="mb-2">
                <Label for="reply_message" class="text-left">
              Broadcast Message
            </Label>
            </div>
...
//   }, 300)
  ...
  const confirmBroadcast = async () => {
    for (const recipient of recipients.value) {
      ...
      try {
        if (await $xmtpClient.value.canMessage(recipient.xmtp_address)) {
          const conversation = await $xmtpClient.value.conversations.newConversation(recipient.xmtp_address);
          conversation.send(inputMessage.value);
